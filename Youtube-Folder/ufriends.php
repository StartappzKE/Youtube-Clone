...
if($page=="")
$page = 1;
$sql = "SELECT count(*) as total from friends WHERE UID=$_REQUEST[UID] and friends_status='Confirmed' limit $config[total_per_ini]";
...
$sql="select * from friends WHERE UID=$_REQUEST[UID] and friends_status='Confirmed' limit $startfrom, $config[items_per_page]";
$rs=$conn->execute($sql);
...
$page_link="";
for($k=1;$k<=$tpage;$k++) $page_link.="<a href='ufriends.php?UID=$_REQUEST[UID]&page=$k&type=$_REQUEST[type]'>$k</a>&nbsp;&nbsp;";
//END PAGING
